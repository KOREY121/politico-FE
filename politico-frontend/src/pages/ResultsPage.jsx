...
export default function ResultsPage() {
  ...
  // ── BUILD DONUT SVG ──
  function buildDonut(results, total) {
  ...
  const donut = results?.total_votes > 0
    ? buildDonut(results.results, results.total_votes)
    : null;
      ...
      <div className="page-wrap">
        ...
        <div className="page-header">
          <div className="page-header-left">
            ...
            <div className="sub">Live counts · Updated in real time</div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', fontFamily: "'DM Mono',monospace", fontSize: '.65rem', color: 'var(--success)' }}>
          ...
          <div className="results-layout">
            ...
            <div>
              ...
              {results.total_votes === 0 && (
                <div style={{ background: 'var(--card)', border: '1px solid var(--border)', borderRadius: '4px', padding: '1.5rem', marginBottom: '1.5rem', fontFamily: "'DM Mono',monospace", fontSize: '.72rem', color: 'var(--muted)' }}>
                        ...
                        {candidates.map(c => {
                          ...
                          return (
                            <div key={c.candidate_id} style={{ marginBottom: '.8rem' }}>
                              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '.78rem', marginBottom: '4px' }}>
                                <span>{c.full_name}</span>
                                <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.68rem', color: 'var(--muted)' }}>{p}%</span>
                              </div>
                              <div className="result-bar-wrap">
            ...
            <div>
              ...
              <div className="card" style={{ marginBottom: '1rem' }}>
                ...
                {!donut
                  ? <div style={{ fontFamily: "'DM Mono',monospace", fontSize: '.65rem', color: 'var(--muted)' }}>No votes yet</div>
                  : <>
                      ...
                      <div className="legend">
                        {donut.sorted.map(([party, count], i) => (
                          <div key={party} className="legend-item">
                            ...
                            <span>{party}</span>
                            <span style={{ marginLeft: 'auto', fontFamily: "'DM Mono',monospace", fontSize: '.65rem', color: 'var(--muted)' }}>
              ...
              <div className="card">
                ...
                {[
                  ['Status',     <span className={`badge badge-${results.election_status}`}>{results.election_status}</span>],
                  ['Start',      results.start_date],
                  ['End',        results.end_date],
                  ['Candidates', results.total_candidates],
                ].map(([key, val]) => (
                  <div key={key} style={{ display: 'flex', justifyContent: 'space-between', padding: '6px 0', borderBottom: '1px solid var(--border)', fontSize: '.82rem' }}>
                    <span style={{ color: 'var(--muted)', fontFamily: "'DM Mono',monospace", fontSize: '.62rem', textTransform: 'uppercase', letterSpacing: '1px' }}>{key}</span>
                    <span style={{ fontFamily: "'DM Mono',monospace", fontSize: '.72rem' }}>{val}</span>
                  </div>
                ))}
              </div>
